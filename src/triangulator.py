...
def removeBadTriangles(badTriangles, triangulation):
    # This loop removes the bad triangles from the triangulation.
...
def addNewTriangles(polygon, point, triangulation):
    # This loop adds a new triagle to the triangulation for every edge in the polygon.
    for edge in polygon:
...
def BowyerWatson(roomCenterPoints, sizeX, sizeY):
    ...
    for point in roomCenterPoints:
        # This function selects the bad triangles from the triangulation.
        badTriangles = badTrianglesList(triangulation, point)
        # This function generates the polygon from the bad triangles.
